...
function AchievementsPlus:Initialize ()
	...
	local achievements = file.FindInLua ("achievementsplus/achievements/*.lua")
...
function AchievementsPlus:AddEventHook (event, name, func, ach)
	if !AchievementsPlus.EventHooks [event] then
		AchievementsPlus.EventHooks [event] = {}
	end
	if !AchievementsPlus.EventHooks [event] [name] then
...
function AchievementsPlus:CallHook (name, arg1, ...)
	if !self.EventHooks [name] then
		return
	end
	for _, v in pairs (self.EventHooks [name]) do
		if !v.ach or (v.ach and self.Achievements [v.ach].count != self.Achievements [v.ach].goal) then
			v.func (arg1, ...)
...
function AchievementsPlus:HookFunctions ()
	if !self.OldusermessageIncomingMessage then
...
function AchievementsPlus:HookAchievements ()
	if !self.OldachievementsCount then
