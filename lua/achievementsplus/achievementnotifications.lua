...
function AchievementsPlus.Notifications:Add (title, text, material)
	table.insert (self.NotificationQueue, {title = title, text = text, material = material})
end

function AchievementsPlus.Notifications:Pop ()
	table.remove (self.NotificationQueue, table.GetFirstKey (self.NotificationQueue))
end
...
function NOTIFICATIONPANEL:Think ()
	if !self.Notification then
		...
		self.Notification = table.GetFirstValue (AchievementsPlus.Notifications.NotificationQueue)
		if self.Notification then
